...
- **Cross-language spec generation** — the source generator also emits an `{Name}Specs` static class with `OpenApiJson`, `OpenApiYaml`, and `Wsdl` string properties; write them to disk to generate client stubs in any language via OpenAPI Generator or swagger-codegen
...
## Cross-Language Spec Generation

The source generator automatically emits a companion `{ProtoFileName}Specs` static class alongside
every generated C# file. It contains three pre-built string properties:

| Property | Format | Description |
|---|---|---|
| `OpenApiJson` | OpenAPI 3.0 JSON | Suitable for Swagger UI, Postman, and code generators |
| `OpenApiYaml` | OpenAPI 3.0 YAML | Human-readable equivalent of `OpenApiJson` |
| `Wsdl` | WSDL 1.1 XML | For SOAP-based tooling and older enterprise systems |

### Generated class

Given `echo.proto`, the generator emits:

```csharp
// <auto-generated/>
namespace MyApp.Messages
{
    public static partial class EchoSpecs
    {
        public static string OpenApiJson => @"{ ""openapi"": ""3.0.0"", ... }";
        public static string OpenApiYaml => @"openapi: '3.0.0' ...";
        public static string Wsdl        => @"<definitions ...>...</definitions>";
    }
}
```
...
Write the spec files from any startup code, test, or build step:

```csharp
using System.IO;
using MyApp.Messages;

// Write once at startup or in a build task
File.WriteAllText("openapi.json", EchoSpecs.OpenApiJson);
File.WriteAllText("openapi.yaml", EchoSpecs.OpenApiYaml);
File.WriteAllText("echo.wsdl",    EchoSpecs.Wsdl);
```

### Generating cross-language client stubs

With the spec files on disk, use standard tooling to generate clients in any language:

```bash
# OpenAPI Generator CLI (Java, Python, TypeScript, Go, Ruby, ...)
...
swagger-codegen generate -i openapi.yaml -l javascript -o ./clients/js
```

### OpenAPI path conventions

The generator maps proto messages to HTTP endpoints by name suffix:

| Message suffix | HTTP verb | Path |
|---|---|---|
| `Request` | `POST` | `/{MessageName}` |
| `Reply` / `Response` | (response body) | — |
| anything else | (schema only) | — |

Reply messages are paired with their corresponding request by convention:
`GetUserRequest` → `GetUserReply` or `GetUserResponse`.

### WSDL service address

The generated WSDL uses a placeholder service address (`http://localhost:9000/rpc`).
Update it before distributing to external consumers:

```csharp
string wsdl = EchoSpecs.Wsdl
    .Replace("http://localhost:9000/rpc", "https://api.example.com/rpc");
File.WriteAllText("echo.wsdl", wsdl);
```
